...
class ProtocolExecutor: ObservableObject {
    ...
    func executeProtocol(_ protocolDef: ProtocolDefinition) async -> ProtocolExecutionResult {
        ...
        log("🎬 Starting protocol execution: \(protocolDef.protocolName)")
        log("📋 Description: \(protocolDef.description)")
        log("🏷️ Tags: \(protocolDef.tags.joined(separator: ", "))")
        log("📊 Total steps: \(protocolDef.steps.count)")
        ...
        do {
            ...
            for (index, step) in protocolDef.steps.enumerated() {
                ...
                do {
                    ...
                    log("❌ Step \(index + 1) failed: \(error.localizedDescription)")
                    
                    if shouldStopOnError(for: step, error: error) {
                        log("🛑 Critical error, stopping protocol execution.")
            ...
            log("❌ Protocol execution failed during setup: \(error.localizedDescription)")
        }
        ...
        log("🏁 Protocol execution completed")
        log("📊 Results: \(successfulSteps)/\(protocolDef.steps.count) steps successful")
        log("⏱️ Total time: \(String(format: "%.2f", totalExecutionTime))s")
    ...
    private func ensureLogicProReady() async throws {
        ...
        log("✅ Logic Pro is ready for protocol execution")
    }
